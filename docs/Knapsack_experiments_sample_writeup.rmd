...
```{r}
data_5_runs <- read.csv("../data/data_5_runs_combined.txt", sep="")
...
```{r}
negs <- subset(data_5_runs, Score<0)
nrow(negs)
unique(negs$Problem)
```
...
```{r}
pairwise.wilcox.test(data_5_runs$Non_negative_score, data_5_runs$Search_method)
```
...
```{r}
plot(data_5_runs$Non_negative_score ~ data_5_runs$Problem,
     xlab="Searcher", ylab="Score")
```
...
rp <- rpart(Non_negative_score ~ Search_method + Problem + Max_evals, data=data_5_runs)
rp
